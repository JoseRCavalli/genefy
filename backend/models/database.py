...
class Female(Base):
    ...
    birth_date = Column(String(20))  # Mudado para String temporariamente
    ...
    def to_dict(self, complete=False):
        ...
        base = {
            ...
            'name': self.name,
            'birth_date': self.birth_date if isinstance(self.birth_date, str) else (self.birth_date.isoformat() if self.birth_date else None),
            'breed': self.breed,
        ...
        if not complete:
            # Versão resumida - apenas índices principais
            base['main_indices'] = {
                'milk': self.milk,
        ...
        # Versão completa - todos os índices organizados
        base.update({
            'genomic': {
                'genomic_inbreeding': self.genomic_inbreeding,
                'genomic_future_inbreeding': self.genomic_future_inbreeding,
                'test_type': self.test_type,
                'cdcb': self.cdcb
            },
            'pedigree': {
                'sire_reg': self.sire_reg,
                'sire_naab': self.sire_naab,
                'sire_name': self.sire_name,
                'dam_reg': self.dam_reg,
                'dam_id': self.dam_id,
                'mgs_reg': self.mgs_reg,
                'mgs_naab': self.mgs_naab,
                'mgs_name': self.mgs_name
            },
            'production': {
                'milk': self.milk,
                'protein': self.protein,
            ...
            'economic': {
                'net_merit': self.net_merit,
                'tpi': self.tpi,
                'cheese_merit': self.cheese_merit,
                'fluid_merit': self.fluid_merit,
                'grazing_merit': self.grazing_merit,
                'jpi': self.jpi,
                'eco_dollars': self.eco_dollars
            },
            'fertility': {
                'dpr': self.dpr,
                'heifer_conception_rate': self.heifer_conception_rate,
                'cow_conception_rate': self.cow_conception_rate,
                'fertility_index': self.fertility_index,
                'early_first_calving': self.early_first_calving
            },
            'health': {
                'health_index': self.health_index,
                'heifer_livability': self.heifer_livability,
                'livability': self.livability,
                'scs': self.scs,
                'mastitis': self.mastitis,
                'metritis': self.metritis,
                'displaced_abomasum': self.displaced_abomasum,
                'milk_fever': self.milk_fever,
                'retained_placenta': self.retained_placenta,
                'ketosis': self.ketosis
            },
            'type': {
                'ptat': self.ptat,
                'udc': self.udc,
            ...
            'efficiency': {
                ...
                'rfi': self.rfi,
                'ecofeed_life': self.ecofeed_life,
                'ecofeed_heifer': self.ecofeed_heifer,
                'ecofeed_cow': self.ecofeed_cow,
                'eco2feed': self.eco2feed,
                'rci': self.rci,
                'doi': self.doi
            },
            'calving': {
                'daughter_calving_ease': self.daughter_calving_ease,
                'sire_calving_ease': self.sire_calving_ease,
                'daughter_stillbirth': self.daughter_stillbirth,
                'sire_stillbirth': self.sire_stillbirth
            },
            'genotypes': {
                'beta_casein': self.beta_casein,
                'kappa_casein': self.kappa_casein,
                'blg_betalacto': self.blg_betalacto,
                'dgat': self.dgat,
                'dominant_red': self.dominant_red,
            ...
            'other': {
                'productive_life': self.productive_life,
                'milking_speed': self.milking_speed,
                'ooc': self.ooc
            },
