...
# Forçar encoding UTF-8 para evitar erros com caracteres especiais
...
class DataImporter:
    ...
    def import_females_from_excel(self, excel_path: str, user: str = 'Pedro') -> Dict:
        ...
        try:
            # Ler Excel com engine openpyxl para melhor suporte UTF-8
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    for col in df.columns:
                        val = row[col]
                        if pd.notna(val):
                            # Usar conversão segura para UTF-8
                            if isinstance(val, (int, float)):
                                genetic_data[col] = float(val)
                            else:
                                genetic_data[col] = self._safe_str_utf8(val)
            ...
            print(f"  Sem mudancas: {stats['unchanged']}")
            if stats['errors']:
    ...
    def _safe_str_utf8(self, value) -> str:
        """Converte para string com tratamento UTF-8 para caracteres especiais"""
        if value is None or pd.isna(value):
            return None
        try:
            text = str(value)
            # Encode e decode para remover caracteres problemáticos
